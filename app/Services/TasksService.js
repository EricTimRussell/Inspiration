...
class TasksService {
  async completeToDo(id) {
    ...
    await SandboxServer.put(`/Eric/todos/${id}`, todo)
    appState.emit('todos')
  }
  async deleteToDo(id) {
    const yes = await Pop.confirm('Delete ToDo?')
    if (!yes) { return }
    await SandboxServer.delete(`Eric/todos/${id}`)
    appState.todos = appState.todos.filter(t => t.id != id)
    appState.emit('todos')
  }
  async getToDoList() {
    const res = await SandboxServer.get('/Eric/todos')
    appState.todos = res.data.map(r => new Task(r))
    console.log(appState.todos);
  }
  async addToDo(formData) {
    const res = await SandboxServer.post('/Eric/todos', formData)
